...
const TrainingPlanPage: React.FC = () => {
  ...
  const weeks: { sessionType: string; level: number }[][] = [];
    ...
    <div className="bg-background min-h-screen">
      ...
      <main className="p-4 sm:p-6 md:p-8 flex justify-center">
        <Card className="max-w-4xl w-full">
          ...
          <p className="text-lg text-slate-600 mb-8">{t('training_plan_desc')}</p>
              ...
              <div key={weekIndex}>
                ...
                <div className="grid sm:grid-cols-3 gap-4">
                  {week.map((session, sessionIndex) => {
                    const overallSessionIndex = weekIndex * 3 + sessionIndex;
                    const isCompleted = overallSessionIndex < participant.sessions_completed;
                    const isNext = overallSessionIndex === participant.sessions_completed;

                    return (
                      <div 
                        key={sessionIndex} 
                        className={`p-4 rounded-lg border-2 ${isNext ? 'bg-secondary/10 border-secondary' : 'bg-slate-50 border-slate-200'} ${isCompleted ? 'opacity-60' : ''}`}
                      >
